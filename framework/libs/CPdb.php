...
class CPdb {
    ...
    */
    ...
    private function generate_query_sql($table, $pArr)
    {
        ...
        $sql    = "select {$fields} from {$table} where 1=1 {$where} {$order} {$having} limit {$start},{$limit}";
        return $sql;
    }
    ...
    *                field => value,...)
    */
    function delete($table, $cdtArr)
    {
        $table = '`'.trim($table, '`').'`';
        foreach($cdtArr as &$val) {
            $val = "'".$val."'";
        }
        ini_set('arg_separator.output', ' and ');
        $cdtion = http_build_query($cdtArr);
        $cdtion = urldecode($cdtion);
        $sql = "delete from $table where 1=1 and $cdtion";
        $this->sqls[] = $sql;
    ...
    */
    public function update($table, $valArr, $cdtions)
    {
        ...
        $sql = "update {$table} set {$vallist} where 1=1 {$cdtion}";
        $this->execute('set names utf8');
    ...
    */
    ...
    {
        return $this->insert($table, $dataArr, true);
    }
    ...
    */
    public function ftFields($fields, $descArr)
    {
        ...
        if('*' != $fds) {
            ...
            foreach($fArr as $k=>&$f){
                $matchs = preg_match("/[0-9a-z_]{2,}?\(.+\)/si", $f); //说明有函数                
